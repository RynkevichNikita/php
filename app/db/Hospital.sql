...
--
-- Структура таблицы `Patient's card`
--

CREATE TABLE `Patient's card` (
  `id` int NOT NULL,
  `age` int NOT NULL,
...
--
-- Индексы таблицы `Patient's card`
--
ALTER TABLE `Patient's card`
  ADD PRIMARY KEY (`id`),
...
--
-- AUTO_INCREMENT для таблицы `Patient's card`
--
ALTER TABLE `Patient's card`
  MODIFY `id` int NOT NULL AUTO_INCREMENT;
...
--
-- Ограничения внешнего ключа таблицы `Patient's card`
--
ALTER TABLE `Patient's card`
...
--
ALTER TABLE `Patients`
  ADD CONSTRAINT `id` FOREIGN KEY (`id`) REFERENCES `Patient's card` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
COMMIT;
